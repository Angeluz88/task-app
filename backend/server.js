...
const { Pool } = require('pg'); // Usamos pg en lugar de better-sqlite3
...
// Neon requiere SSL. Si usas DATABASE_URL, pg lo detecta, pero forzamos ssl: true por seguridad.
const pool = new Pool({
...
pool.connect((err, client, release) => {
  if (err) {
    ...
    release();
    initDB(); // Inicializar tablas solo si conecta bien
  }
});
...
async function initDB() {
  try {
    ...
    // Seed Data (Verificar si existen frases)
    const check = await pool.query('SELECT count(*) FROM motivational_phrases');
    if (parseInt(check.rows[0].count) === 0) {
      console.log('🌱 Insertando datos de ejemplo...');
      await pool.query(`INSERT INTO motivational_phrases (phrase, category) VALUES 
        ('¡Tú puedes con esto!', 'before_task'),
        ('¡Lo lograste!', 'after_task'),
        ('Eres único', 'general')`);
      
      await pool.query(`INSERT INTO neurodivergence_info (title, content, category) VALUES 
        ('TDAH y Creatividad', 'Muchos genios tenían TDAH.', 'curiosity'),
...
// ==========================================
// RUTAS (Adaptadas a sintaxis Postgres)
// ==========================================
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'Registrado', token, user: { id: result.rows[0].id, name, email, role: 'parent' } });
  } catch (error) {
    console.error(error);
...
app.get('/api/tasks/child/:childId', async (req, res) => {
  ...
  try {
    // Postgres: DATE(completed_at) = CURRENT_DATE
    const tasks = await pool.query(`
      SELECT t.id, t.title, t.description, t.duration_minutes, t.points, t.created_at,
             (SELECT COUNT(*) FROM task_progress tp 
              WHERE tp.task_id = t.id AND tp.child_id = $1 AND DATE(tp.completed_at) = CURRENT_DATE) as completed_today
      FROM tasks t
...
app.post('/api/tasks/complete', async (req, res) => {
  ...
  try {
    ...
    const existing = await pool.query(
      'SELECT id FROM task_progress WHERE task_id = $1 AND child_id = $2 AND DATE(completed_at) = CURRENT_DATE',
      [task_id, child_id]
    );
...
app.post('/api/prizes', authenticateToken, async (req, res) => {
  ...
  try {
    const finalTarget = target_child_id && target_child_id !== 'null' ? target_child_id : null;
    const result = await pool.query(
      ...
      [title, description || '', required_points, reward_type, req.user.id, finalTarget]
    );
...
app.get('/api/prizes/child/:childId', async (req, res) => {
  ...
  try {
    ...
    const processed = prizes.rows.filter(p => {
      if (p.target_child_id !== null && p.target_child_id !== undefined) {
        return parseInt(p.target_child_id) === parseInt(childId);
      }
      return true;
...
app.get('/api/phrases', async (req, res) => {
  ...
  try {
    ...
    if (phrases.rows.length === 0) return res.json({ phrase: "¡Tú puedes!" });
