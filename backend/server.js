...
// Configuración optimizada para eliminar advertencias SSL y mejorar estabilidad
const pool = new Pool({
  connectionString: databaseUrl,
  ssl: {
    rejectUnauthorized: false // Configuración correcta para Neon/Render
  },
  max: 20,
...
// Función para iniciar DB y crear tablas
async function initDB() {
  let client;
  try {
    client = await pool.connect();
    console.log('✅ Conectado exitosamente a Neon PostgreSQL (SSL seguro)');
...
// ==========================================
// RUTAS DE TAREAS (SOLUCIÓN DEFINITIVA DE FECHAS)
// ==========================================
...
app.get('/api/tasks/child/:childId', async (req, res) => {
  ...
  try {
    // SOLUCIÓN: Usar DATE_TRUNC para comparar solo el día en UTC, ignorando horas y zonas horarias
    const result = await pool.query(`
...
// ==========================================
// RUTAS DE PROGRESO (SOLUCIÓN DEFINITIVA DE FECHAS)
// ==========================================
...
app.post('/api/tasks/complete', async (req, res) => {
  ...
  try {
    // SOLUCIÓN: Verificar existencia usando DATE_TRUNC en UTC
...
app.get('/api/scores/:childId', async (req, res) => {
  ...
  try {
    ...
    // Cálculo de rangos en JS para semanal/mensual, y DATE_TRUNC para diario
    ...
    // Diario: Usar DATE_TRUNC consistente con las otras rutas
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`💾 Conectado a Neon PostgreSQL (SSL Verificado)`);
});